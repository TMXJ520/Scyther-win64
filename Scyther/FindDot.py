...
def scanLocations():
    if sys.platform.startswith("win"):
        prefixes = ["", \
                    "D:\Program Files\Graphviz*", \
                    "D:\Program Files (x86)\Graphviz*" ]
        name = "dot.exe"
    else:
        prefixes = [""]
        name = "dot"
...
def findDot():
    ...
    # ✅ 手动指定 dot 路径（请根据实际情况修改）
    manual_path = r"D:\Program Files\Graphviz\bin\dot.exe"

    # 检查路径是否存在
    if os.path.exists(manual_path):
        DOTLOCATION = '"%s"' % manual_path  # 加引号以防路径中含空格
        return DOTLOCATION

    # 否则回退原扫描逻辑
    DOTLOCATION = scanLocations()
    if DOTLOCATION is None:
        Misc.panic("""
