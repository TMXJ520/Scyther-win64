...
class Scyther(object):
    ...
    def doScytherCommand(self, spdl, args, checkKnown=False, storePopen=None):
        ...
        m = hashlib.sha256()
        if spdl == None:
            m.update("[spdl:None]".encode())
        else:
            m.update(spdl.encode())
        if args == None:
            m.update("[args:None]".encode())
        else:
    ...
    def doScytherCommandReal(self, spdl, args, storePopen=None):
        ...
        if spdl:
            ...
            # Write (input) file
            with os.fdopen(fdi, 'w+b') as fhi:
                fhi.write(spdl.encode('utf-8'))

        # Generate command line for the Scyther process
        self.cmd = f'"{self.program}" --append-errors={fne} --append-output={fno} {args}'
        if spdl:
            self.cmd += f' {fni}'
        ...
        # ⬇ 再使用 open(filename) 重新读取内容（避免 fd 被占用问题）
        with open(fne, "r", encoding="utf-8", errors="ignore") as fhe:
            errors = fhe.read()

        with open(fno, "r", encoding="utf-8", errors="ignore") as fho:
            output = fho.read()
        ...
        try:
            ...
            print(f"Warning: 无法删除错误文件 {fne}：{e}")

        try:
            os.remove(fno)
        except PermissionError as e:
            print(f"Warning: 无法删除输出文件 {fno}：{e}")

        if spdl:
            try:
                os.remove(fni)
            except PermissionError as e:
                print(f"Warning: 无法删除输入文件 {fni}：{e}")
